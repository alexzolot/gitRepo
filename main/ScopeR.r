...
#library('zBase') detach(package:zBase)
...
RunJob0= function(TheCodeFolder= 'M:/93_AegisPubScore', ScopeFolder = options()$ScopeFolder
					, msalias= options()$msalias, vc= options()$vc, prior= options()$prior
...
##########################################################
RunJob00= function(LocalRoot= 'M:/test', CosmosRoot= "/my/test"
...
RunJob= function(LocalRoot= 'M:/test', CosmosRoot= "/my/test"
		...
		, RRep= '../dummy.r', RepCopyToDir= ''
		, TimeWindow='3h'
		, LastDay= ch(Sys.Date()-1), toDebug= F, ShivaAeg= FALSE, patt='[tc]sv$', pattNeg='_', ...) { 
	#TimeWindow='7h'
	Timy=   ifelse(grepl('[hH]$',TimeWindow),'hour', 'dai') 
	ndays=  ifelse(grepl('[hH]$',TimeWindow), 1, nu(gsub('.$','',TimeWindow)))
	nhours= if(grepl('[hH]$',TimeWindow)) nu(gsub('.$','',TimeWindow)) else 24
...
#	str(argss)
	...
	if(State$state== 'Init'){
		...
		if(1)s= transform.scopeScript(script=ScopeTempl.script, fileShow= T
					, hourly=(Timy=='hour'), outScript= gsub('.*?([^\\/]*)$', '\\1', f) , outScriptDir=gw()
					, StartDate= sf('@"%s"', LastDay- ndays + 1)
					, EndDate=sf('@"%s"', LastDay)
					, EndHour= sf('@"%02d"', 8+nhours)
					, root= sf('@"%s/"', CosmosOut)
					, Timy=sf('@"%s"',Timy))
...
#                            ,  msalias, scName, f))
		...
		if(ShivaAeg){
			vers=TimeWindow %+% '-' %+%  Job.Version
			s= transform.scopeScript(script=ScopeTempl.script, fileShow= T
					, hourly= (Timy=='hour')
					# , outScript= gsub('.*?([^\\/]*)$', '\\1', f) 
					, outScriptDir= gw()
					#, outScriptFile= AV_script<<- sf('%s/AegisCustomDataSourceView.%s.script', gw(), vers)
#							, p_Start_Date=sf('@"%s"', LastDay- ndays + 1)
#							, p_End_Date=sf('@"%s"', LastDay)
					, outScriptFile= ''
					, StartDate= sf('@"%s"', LastDay- ndays + 1), EndDate=sf('@"%s"', LastDay)
					, EndHour= sf('@"%02d"', 8+nhours)
					, root= sf('@"%s/"', CosmosOut)
					...
					, p_Start_Date=sf('%s', LastDay- ndays + 1)
					, p_End_Date=sf('%s', LastDay)
					, p_PubScoring_Start_Date= LastDay
					, p_AZ_Comment= sf('LocalPath=M:/73_ShivaAegisAdjust/work.b/%s.%s', LastDay,Job.Version), ...)  # default 130 15 # gff('Threshold *=',,
			
			
			s= gsub("@@p_Bing_DataSource@@", "uber", s)
			s= gsub("@@p_CI_VERSION@@", "ci/33777", s)
			# not used?  s= gsub("@@p_CurrentMinusTwoDateInDateTime@@", "", s)
			s= gsub("@@p_DATA_SOURCE@@", "PROD", s)
			s= gsub("@@p_DayMinus2@@"  , format(LastDay- 2,"%d"), s)
			s= gsub("@@p_MonthMinus2@@", format(LastDay- 2,"%m"), s)
			s= gsub("@@p_YearMinus2@@" , format(LastDay- 2,"%Y"), s)
			s= gsub("@@p_CFR_Internal_DSV@@", "metadataCFR_Internal__9_5.dsv", s)
			s= gsub("@@p_DE_MR_DSV@@", "metadataDE_MR__10_1.dsv", s)
			s= gsub("@@p_EnableDebugging@@", "true", s)   ######<<<---
			#s= gsub("@@p_End_Date@@", "2011-12-07", s)    ######<<<---
			s= gsub("@@p_KVP_VIEW_PATH@@", "/shares/adCenter.BICore.views/apsviews/PROD", s)
			s= gsub("@@p_PubCollOutputDir@@"         , sf('"%s"', CosmosOut), s)  ######<<<--- xxx ??
			s= gsub("@@p_PublisherScoringOutputDir@@", sf('"%s"', CosmosOut), s) ######<<<---
			s= gsub("@@p_Publisher_Collusion_ClickCount_Threshold@@", "1", s)
			#s= gsub("@@p_PubScoring_Start_Date@@", "2011-12-07", s)  ######<<<---
			#  String.Format(@"{0}/{1}", @AegisStatsLocation, "ReferenceDataFiles");	 
			s= gsub("@@p_ReferenceDataFilesRoot@@", "/my/prod/pipelines/Aegis/ReferenceDataFiles", s)
			s= gsub("@@p_SideStreamsLocation@@", "/shares/searchWebLoad/IndexV2/sidestream", s)
			s= gsub("@@p_SplitLogsRootPath@@", "/shares/adCenter.BICore.prod2/prod/splitlogs", s)
			#s= gsub("@@p_Start_Date@@", "2011-12-07", s)  ######<<<---
			s= gsub("@@p_StreamExpiry@@", "20", s)
			
			s= gsub("EXPORT", "/* AZ gsub EXPORT */", s)
			#			Error submitting job.
			#			
			#			VcClientExceptions.VcClientException: [HttpStatusCode = 0; Description = Could not get stream info 
			#	for http://cosmos04.osdinfra.net:88/cosmos/adCenter.BICore/shares/adCenter.BICore.views/apsviews/PROD/SA_KVP_daily_view.script
			#			[HttpStatusCode = 0; Description = The operation has timed out 
			#			WebException Status = Timeout
			#			VcClient request did not reach the VCWS. This could be because of the network issues between your machine and the VCWS or due to proxy issues]
			#			
			#prr(s)
			
			# s= gsub("@IPH2ClickCountThreshold", "7", s) # default 15
			
			#writeLines(he(s,9999), con ='M:/73_ShivaAegisAdjust/work.b/AegisCustomDataSourceView.script') 
			writeLines(he(s,9999), con ='F:/idss/Aegis/bin/AegisCustomDataSourceView.script') 
			file.copy('F:/idss/Aegis/bin/AegisCustomDataSourceView.script' # expl('M:/73_ShivaAegisAdjust/work.b/')  AegisCustomDataSourceView.script')
					, AV_script<<- sf('%s/AegisCustomDataSourceView.%s.%s.script', gw(), LastDay, vers)
					, overwrite =T)
			catt('295. AV_script=', AV_script)
			#file.copy('M:/73_ShivaAegisAdjust/work/AegisCustomDataSourceViewI.script'
			#		, sf('F:/idss/Aegis/bin/AegisCustomDataSourceView.script', vers), overwrite =T)
			
			Upload2Cosmos(CosmosDir='/my/prod/pipelines/Aegis/'
					, LocDir='F:/idss/Aegis/bin',  patt='AegisCustomDataSourceView\\.script'
					, vc= 'http://cosmos04.osdinfra.net:88/cosmos/adCenter.BICore')	 
			
			#sh= exec('F:/idss/Aegis/bin/zAlexRocks.cmd')  # expl('F:/idss/Aegis/bin/') # sw('M:/73_ShivaAegisAdjust/idss/Aegis/bin/')
			sh= exec('F:/idss/Aegis/bin/zAlexRocks2.cmd')  # expl('F:/idss/Aegis/bin/') # sw('M:/73_ShivaAegisAdjust/idss/Aegis/bin/')
			
			if(0){
				# http://cosmos04.osdinfra.net:88/cosmos/adCenter.BICore/_Jobs/3d8a73f7-06ba-4bbd-8a79-cf31b5ffbda3/Resources/scoperuntime.exe) 
				# http://cosmos04.osdinfra.net:88/cosmos/adCenter.BICore/my/73_ShivaAegisAdj/2011-11-01.L-1d/Debug/DistributionReference.csv
				
				# http://cosmos04.osdinfra.net:88/cosmos/adCenter.BICore/my/jobs/{0210fa66-2186-4e4b-ae44-60bf511b6100}/metadataCFR_Internal__9_5.dsv
				
				dir.scope(ScopeFolder = options()$ScopeFolder
						, CosmosDir='my/73_ShivaAegisAdj/2011-11-01.L-1d/Debug'
						, vc= 'http://cosmos04.osdinfra.net:88/cosmos/adCenter.BICore')
				
				dir.scope(ScopeFolder = options()$ScopeFolder
						, CosmosDir='my/jobs/{3d8a73f7-06ba-4bbd-8a79-cf31b5ffbda3}'
						, vc= 'http://cosmos04.osdinfra.net:88/cosmos/adCenter.BICore')
				
				dir.scope(ScopeFolder = options()$ScopeFolder
						, CosmosDir='_Jobs/3d8a73f7-06ba-4bbd-8a79-cf31b5ffbda3'
						, vc= 'http://cosmos04.osdinfra.net:88/cosmos/adCenter.BICore')
				
				dir.scope(ScopeFolder = options()$ScopeFolder
						, CosmosDir='Jobs/3d8a73f7-06ba-4bbd-8a79-cf31b5ffbda3/Resources'
						, vc= 'http://cosmos04.osdinfra.net:88/cosmos/adCenter.BICore')
				DownloadScopeFiles(ScopeFolder = options()$ScopeFolder
						, CosmosDir='_Jobs/3d8a73f7-06ba-4bbd-8a79-cf31b5ffbda3/Resources'
						, vc= 'http://cosmos04.osdinfra.net:88/cosmos/adCenter.BICore'
						, LocDir = "M:/73_ShivaAegisAdjust/idss/Aegis/bin/", 
				)
		...
		#brr()
		#if(!any(grepl(' successfully', sh)) ){prr(s); stop('RunJob:Scope: Not successfully!!!')}
		if(!any(grepl(' successfully', sh)) ){prr(sh); stop('RunJob:Scope: Not successfully!!!')} # zzz TODO
	...
	if(State$state== 'ScopeSubmitted'){ # monitor Scope execution ===
		...
		#ii= grep(sf('Job Name.*%s',scName),sh)
		ii= grep(jobId,sh)  #  sort(grep('Job ID', sh, v=T)) 
		#for(i in ii)catt(sh[(i-3):(i+3)])
		for(i in ii)try({print(sh[(i):(i+5)])}, s=T)
	...
	if(State$state== 'ScopeCompletedSuccess'){ # download scope results === xxx
		...
		#download scope files
		#	    if(ShivaAeg){ sc.out= scope(cmd=sf('dir %s/Debug', CosmosOut), ScopeFolder=ScopeFolder, vc=vc)
		#		} else { sc.out= scope(cmd=sf('dir %s', CosmosOut), ScopeFolder=ScopeFolder, vc=vc)}
		sc.out= scope(cmd=sf('dir %s%s', CosmosOut, ifelse(ShivaAeg, '/Debug', '')), ScopeFolder=ScopeFolder, vc=vc)
		
		catf('2481. CosmosOut=%s\n sc.out= %s\n',CosmosOut, sc.out)
	...
	# try({flist=DownloadScopeFiles(ScopeFolder=ScopeFolder
	# 		, CosmosDir= CosmosOut
	# 		, LocDir=getwd(), MaxLen=1e8, patt=patt, pattNeg=pattNeg
	# 		, vc= vc)}, s=T)
	# 
	# if(ShivaAeg){
	# 	try({flist=DownloadScopeFiles(ScopeFolder=ScopeFolder
	# 			, CosmosDir= sf('%s%s', CosmosOut, ifelse(ShivaAeg, '/Debug', ''))
	# 			, LocDir=getwd(), MaxLen=1e8, patt=patt, pattNeg=pattNeg
	# 			, vc= vc)}, s=T)
	# }
...
transform.scopeScript= function(script="M:/83_ScopeR/AegisCustomDataSourceView.script"
	...
	catf('::: s= gsub(\'\', \'\',  s)\n::: writeLines(he(s,9999), con =\'%s\')', outScript1)
	#print(s[80:100])
    if(outScriptFile != '') {writeLines(he(s,9999), con = outScript1)
...
#                len= nu(gsub('.+:(.+)$','\\1', grep('Committed Length', sc.out, v=T)))) 
	...
	if(!file.exists(LocDir))dir.create(LocDir, recursive = T)
	...
	if(le(flist)> 0){
			...
			print(ff)
			invisible(ff)
	} else{catf('No Files from %s /  %s  with Length < %d\nwere downloaded to folder %s !!!\n', vc, CosmosDir, MaxLen, LocDir); return(df())}
}
if(0){
...
#download scope files
Upload2Cosmos= Upload2Scope= function(ScopeFolder= options()$ScopeFolder
		, CosmosDir='/my/tmp/'
		, LocDir='',  patt='[tc]sv$'
		, pattNeg='zzz'
		, vc= options()$vc, overWrite=T){
#        sc.out= shell(sf('%s/scope.exe dir %s -vc %s',
#                        ScopeFolder, CosmosDir, vc), wait=T, intern = T)
#        sc.outStreams= df(stream= gsub('.+(http:.+) $','\\1', grep('Stream', sc.out, v=T)), 
#                file= gsub('.+/([^/]+) $','\\1', grep('Stream', sc.out, v=T)), 
...
#	sink('log.txt', append =T); pr(sc.Streams); sink()
	...
	for(f in sus(ff<- dir(LocDir), grepl(patt, ff) & !grepl(pattNeg, ff))){
		...
		if(overWrite)scope(sf('delete %s/%s/%s', vc,CosmosDir, f))
